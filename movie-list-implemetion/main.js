...
function renderMovieList(data) {
  //title, image
  if (dataPanel.dataset.view === "card-mode") {
    let rawHTML = ''
    data.forEach(item => {
      rawHTML +=
        `
  ...
  } else if (dataPanel.dataset.view === "list-mode") {
  ...
  modeToggler.addEventListener("click", function viewToggling(event) {
    if (event.target.matches("#card-mode")) {
      dataPanel.dataset.view = "card-mode"
      renderMovieList(getMoviesByPage(currentPage))
    } else if (event.target.matches("#list-mode")) {
      dataPanel.dataset.view = "list-mode"
      renderMovieList(getMoviesByPage(currentPage))
    }
  })